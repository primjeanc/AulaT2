...
namespace exercicioVerProfessor
{
    class Program
    {
        static void Main(string[] args)
        {
            Console.WriteLine("Informa a palavra da busca:");
        ...
        /// <summary>
        /// Método de busca detro da aplicação Parametro de até 3 caracteres
        /// </summary>
        ...
        private static int FindByFor(string wordFind)
        {
            ...
            var wordFindCountLength = wordFind.Length; //pondendo trabalhar com valores diferentes de 3 caracteres
            var lengthText = (contentToFind.Length - (wordFindCountLength - 1));
            var contWordFind = 0;

            for (int i = 0; i < lengthText; i++) // ler contentToFind pesaria mais que lenghtText pois acessaria o metodo em cada teste
            {
                var compareWord = string.Empty;
                for (int j = 0; j < wordFindCountLength; j++)
                {
                    compareWord += contentToFind[i + j].ToString(); // add "+" para salvar cada caracter ABC - a-b-c aou inves de C apenas
        ...
        private static int FindByWhile(string wordFind)
        {
            ...
            while (i < lengthText)
            {
                ...
                {
                    compareWord += contentToFind[i + b].ToString(); // add "+" para salvar cada caracter ABC - a-b-c aou inves de C apenas
                    b++;
                }
